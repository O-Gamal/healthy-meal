...
const Home: NextPage = () => {
  ...
  const handleMealTypeFilter = (mealType: string) => {
    if (selectedMealTypes.includes(mealType)) {
      const updatedMealTypesList = selectedMealTypes.filter(meal => meal !== mealType);
      ...
      <main className='w-full space-y-10'>
        ...
        <section className='space-y-5'>
          ...
          <section className='flex justify-center gap-5 items-center'>
            <Switch
              size='md'
              label={<span className='text-white text-lg cursor-pointer select-none'>Following's meals only</span>}
              className=''
            />
            <div className='space-x-2'>
              {
                mealTypes.map(
                  (mealType) => <span key={mealType} onClick={() => handleMealTypeFilter(mealType)} className={`border select-none cursor-pointer px-2 py-1 rounded-md ${selectedMealTypes.includes(mealType) ? 'bg-green border-green text-dark-primary font-semibold' : 'hover:bg-dark-secondary border-white'}`}>{mealType}</span>
                )
              }
            </div>
          </section>
