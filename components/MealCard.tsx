import { BookmarkIcon as BookmarkOutlineIcon, ClockIcon, HandThumbUpIcon, PowerIcon } from '@heroicons/react/24/outline';
...
const MealCard = ({ photo, name, description, prep_time, calories, likes, user }: MealCardProps) => {
        ...
        <div className='w-96 rounded-lg bg-dark-secondary'>
            ...
            <section className='p-5 space-y-8'>
                ...
                <section className='space-y-3'>
                    <h3 className='text-2xl font-bold cursor-pointer hover:opacity-90'>{name}</h3>
                    <p>{description?.slice(0, 100)} ...</p>
                </section>
                <section className='flex justify-between items-center select-none'>
                    <div className="flex items-center gap-1 cursor-pointer">
                        <Image src={user.photo} alt={user.name} width={30} height={30} objectFit='cover' className='rounded-full' />
                        <p className="font-sm">by <span className="hover:underline">{user.name}</span></p>
                    </div>
                    {bookmarked ? <BookmarkSolidIcon className="w-7 h-7 cursor-pointer" onClick={() => setBookmarked(false)} /> : <BookmarkOutlineIcon className="w-7 h-7 cursor-pointer" onClick={() => setBookmarked(true)} />}
                </section>
            </section>
        </div>
    )
